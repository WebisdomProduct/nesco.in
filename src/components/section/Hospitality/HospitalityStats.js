...
// 🔒 HARD-CODED FALLBACK ICONS (DO NOT REMOVE)
...
function HospitalityStats() {
        ...
        if (Array.isArray(res.data?.data) && res.data.data.length > 0) {
          setStats(
            res.data.data.map((item, index) => ({
              value: item.value,
              label: item.label,
              // use CMS icon OR fallback icon safely
              icon:
                item.icon ||
                fallbackStats[index % fallbackStats.length].icon,
            }))
          );
        }
      } catch (err) {
        console.warn("Stats CMS unavailable — using fallback data");
      }
    };
  ...
  // ================= UI =================
  return (
    <div className="w-full h-auto flex items-center justify-center bg-gray-100 py-8 md:py-12 lg:py-16">
      <div className="w-[90%] max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
        {stats.map((stat, index) => (
          ...
          >
            ...
            {/* VALUE */}
            <h3
              className="text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[3.5rem] font-bold text-black"
            ...
            {/* LABEL */}
            <p
              className="text-center text-[0.9rem] sm:text-[1rem] md:text-[1.1rem] text-black mb-4 md:mb-6 lg:mb-8 max-w-[280px]"
